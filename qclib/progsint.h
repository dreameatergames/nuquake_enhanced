...
void *QCHunkAlloc(progfuncs_t *progfuncs, int size);
...
int QC_ToggleBreakpoint(progfuncs_t *progfuncs, char *filename, int linenum, int flag);
...
char *QC_EvaluateDebugString(progfuncs_t *progfuncs, char *key);
char *QC_SaveEnts(progfuncs_t *progfuncs, char *mem, int *size, int mode);
int QC_LoadEnts(progfuncs_t *progfuncs, char *file, float killonspawnflags);
char *QC_SaveEnt (progfuncs_t *progfuncs, char *buf, int *size, struct edict_s *ed);
struct edict_s *QC_RestoreEnt (progfuncs_t *progfuncs, char *buf, int *size, struct edict_s *ed);
char *QC_VarString (progfuncs_t *progfuncs, int	first);
void QC_StackTrace (progfuncs_t *progfuncs);
...
void QC_ExecuteProgram (progfuncs_t *progfuncs, func_t fnum);
int QC_LoadProgs(progfuncs_t *progfncs, char *s, int headercrc, qcbuiltin_t *builtins, int numbuiltins);
int QC_ReallyLoadProgs (progfuncs_t *progfuncs, char *filename, int headercrc, progstate_t *progstate, pbool complain);

void *QCHunkAlloc(progfuncs_t *progfuncs, int ammount);

void QC_Profile_f (void);

struct edict_s *QC_Edict_Alloc (progfuncs_t *progfuncs);
void QC_Edict_Free (progfuncs_t *progfuncs, struct edict_s *ed);

char *QC_NewString (progfuncs_t *progfuncs, char *string, int minlength);
// returns a copy of the string allocated from the server's string heap

void QC_Print (progfuncs_t *progfuncs, struct edict_s *ed);
//void ED_Write (FILE *f, edictrun_t *ed);
char *QC_ParseEdict (progfuncs_t *progfuncs, char *data, edictrun_t *ent);

//void QC_WriteGlobals (FILE *f);
void QC_ParseGlobals (char *data);
...
struct edict_s *QC_EdictNum(progfuncs_t *progfuncs, unsigned int n);
unsigned int QC_NumForEdict(progfuncs_t *progfuncs, struct edict_s *e);
...
const extern	unsigned int		qctype_size[];


extern	unsigned short		pr_crc;

void VARGS QC_RunError (progfuncs_t *progfuncs, char *error, ...);

void QC_PrintEdicts (progfuncs_t *progfuncs);
void QC_PrintNum (progfuncs_t *progfuncs, int ent);


pbool QC_SwitchProgs(progfuncs_t *progfuncs, progsnum_t type);
void QC_MoveParms(progfuncs_t *progfuncs, progsnum_t progs1, progsnum_t progs2);
...
qceval_t *QC_FindGlobal(progfuncs_t *prfuncs, char *globname, progsnum_t pnum);
ddef16_t *QC_FindTypeGlobalFromProgs16 (progfuncs_t *progfuncs, char *name, progsnum_t prnum, int type);
ddef32_t *QC_FindTypeGlobalFromProgs32 (progfuncs_t *progfuncs, char *name, progsnum_t prnum, int type);
ddef16_t *QC_FindGlobalFromProgs16 (progfuncs_t *progfuncs, char *name, progsnum_t prnum);
ddef32_t *QC_FindGlobalFromProgs32 (progfuncs_t *progfuncs, char *name, progsnum_t prnum);
fdef_t *QC_FindField (progfuncs_t *progfuncs, char *name);
fdef_t *QC_FieldAtOfs (progfuncs_t *progfuncs, unsigned int ofs);
dfunction_t *QC_FindFunction (progfuncs_t *progfuncs, char *name, progsnum_t *pnum, progsnum_t fromprogs);
func_t QC_FindFunc(progfuncs_t *progfncs, char *funcname, progsnum_t pnum);
void QC_Configure (progfuncs_t *progfncs, int addressable_size, int max_progs);
int QC_InitEnts(progfuncs_t *progfncs, int maxents);
char *QC_ValueString (progfuncs_t *progfuncs, etype_t type, qceval_t *val);
void QC_ClearEdict (progfuncs_t *progfuncs, edictrun_t *e);
void QCAddressableFlush(progfuncs_t *progfuncs, int totalammount);
void QC_FlushProgsOffsets(progfuncs_t *progfuncs);

ddef16_t *QC_GlobalAtOfs16 (progfuncs_t *progfuncs, int ofs);
ddef16_t *QC_FindGlobal16 (progfuncs_t *progfuncs, char *name);
ddef32_t *QC_FindGlobal32 (progfuncs_t *progfuncs, char *name);
ddef32_t *QC_GlobalAtOfs32 (progfuncs_t *progfuncs, unsigned int ofs);

string_t QC_StringToProgs			(progfuncs_t *inst, char *str);
char *QC_StringToNative				(progfuncs_t *inst, string_t str);

void QC_FreeTemps			(progfuncs_t *progfuncs, int depth);

char *QC_GlobalString (progfuncs_t *progfuncs, int ofs);
char *QC_GlobalStringNoContents (progfuncs_t *progfuncs, int ofs);
