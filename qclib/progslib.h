...
typedef void (*qcbuiltin_t) (progfuncs_t *prinst, struct globalvars_s *gvars);

//your edict structure *must* begin with these fields
...
//you can add anything else you want under here
} edictrun_t;
...
#define sizeofqcevalc sizeof(qcevalc_t)
typedef enum {qct_void, qct_string, qct_float, qct_vector, qct_entity, qct_field, qct_function, qct_pointer, qct_integer, qct_variant, qct_struct, qct_union} etype_t;
...
struct progfuncs_s {
	...
	progsnum_t	(*LoadProgs)			(progfuncs_t *prinst, char *s, int headercrc, qcbuiltin_t *builtins, int numbuiltins);	//load a progs
	...
	struct edict_s	*(*EdictNum)		(progfuncs_t *prinst, unsigned int n);		//get the nth edict
	unsigned int		(*NumForEdict)			(progfuncs_t *prinst, struct edict_s *e);	//so you can find out what that 'n' will be
	...
	union qceval_s	*(*FindGlobal)		(progfuncs_t *prinst, char *name, progsnum_t num);	//find a pointer to the globals value
	...
	union qceval_s	*(*GetEdictFieldValue)	(progfuncs_t *prinst, struct edict_s *ent, char *name, qcevalc_t *s); //get an entityvar (cache it) and return the possible values
	...
	void	(*RegisterBuiltin)			(progfuncs_t *prinst, char *, qcbuiltin_t);
	...
	int (*QueryField)		(progfuncs_t *prinst, unsigned int fieldoffset, etype_t *type, char **name, qcevalc_t *fieldcache);	//find info on a field definition at an offset
	int (*GetFuncArgCount)	(progfuncs_t *prinst, func_t func);
};
...
typedef struct progexterns_s {
	...
	qcbuiltin_t *(*builtinsfor) (int num, int headercrc);	//must return a pointer to the builtins that were used before the state was saved.
	...
	qcbuiltin_t *globalbuiltins;	//these are available to all progs
...
void QC_Edict_Print (progfuncs_t *progfuncs, struct edict_s *ed);
...
#if !defined(DLL_PROG) && !defined(NOSHORTCUTS)
...
#define	QC_EDICT(pf, o) PROG_TO_EDICT(pf, G_INT(o)) //((edict_t *)((char *) sv.edicts+ *(int *)&((float *)pr_globals)[o]))
#define QC_EDICTNUM(pf, o) NUM_FOR_EDICT(pf, G_EDICT(pf, o))
