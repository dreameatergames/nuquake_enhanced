...
{	
	if ((unsigned)type >= maxprogs)
		QC_RunError(progfuncs, "QCLIB: Bad prog type - %i", type);
...
void QC_MoveParms(progfuncs_t *progfuncs, progsnum_t progs1, progsnum_t progs2)	//from 2 to 1
{
	unsigned int a;
	progstate_t *p1;
	progstate_t *p2;
...
progsnum_t QC_LoadProgs(progfuncs_t *progfuncs, char *s, int headercrc, qcbuiltin_t *builtins, int numbuiltins)
{
	unsigned int a;
	progsnum_t oldtype;
	...
	for (a = 0; a < maxprogs; a++)
	{
		if (pr_progstate[a].progs == NULL)
		{
			...
			if (QC_ReallyLoadProgs(progfuncs, s, headercrc, &pr_progstate[a], false))	//try and load it			
			{
				current_progstate->builtins = builtins;
				current_progstate->numbuiltins = numbuiltins;
				if (oldtype>=0)
				QC_SwitchProgs(progfuncs, oldtype);
...
//forget a progs
void QC_Clear(progfuncs_t *progfuncs)
{
	unsigned int a;
	for (a = 0; a < maxprogs; a++)
	{
...
//			printf("Dupfield %s %i -> %i\n", name, field[i].progsofs,field[i].ofs);
	...
	if (type == qct_vector)
	{
		...
		n=QCHunkAlloc(progfuncs, namelen);
		sprintf(n, "%s_x", name);
		ofs = QC_RegisterFieldVar(progfuncs, qct_float, n, engineofs, progsofs);
		field[fnum].ofs = ofs+progfuncs->fieldadjust;

		n=QCHunkAlloc(progfuncs, namelen);
		sprintf(n, "%s_y", name);
		QC_RegisterFieldVar(progfuncs, qct_float, n, (engineofs==-1)?-1:(engineofs+4), (progsofs==-1)?-1:progsofs+1);

		n=QCHunkAlloc(progfuncs, namelen);
		sprintf(n, "%s_z", name);
		QC_RegisterFieldVar(progfuncs, qct_float, n, (engineofs==-1)?-1:(engineofs+8), (progsofs==-1)?-1:progsofs+2);
	}
	else if (engineofs >= 0)
...
//	if (type != ev_vector)
		if (fields_size < (ofs+qctype_size[type])*4)
			fields_size = (ofs+qctype_size[type])*4;
