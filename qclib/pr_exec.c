...
*/
/*
void QC_PrintStatement (progfuncs_t *progfuncs, dstatement16_t *s)
{
	int		i;
printf("QC_PrintStatement is unsupported\n");
return;
	...
	if (s->op == OP_IF || s->op == OP_IFNOT)
		printf ("%sbranch %i",QC_GlobalString(progfuncs, s->a),s->b);
	...
	{
		printf ("%s",QC_GlobalString(progfuncs, s->a));
		printf ("%s", QC_GlobalStringNoContents(progfuncs, s->b));
	}
	else
	{
		if (s->a)
			printf ("%s",QC_GlobalString(progfuncs, s->a));
		if (s->b)
			printf ("%s",QC_GlobalString(progfuncs, s->b));
		if (s->c)
			printf ("%s", QC_GlobalStringNoContents(progfuncs, s->c));
	}
	printf ("\n");
}
*/
...
int QC_EnterFunction (progfuncs_t *progfuncs, dfunction_t *f, int progsnum)
{
	int		i, j, c, o;
...
// save off any locals that the new function steps on (to a side place, fromwhere they are restored on exit)
	...
	if (localstack_used + c > LOCALSTACK_SIZE)
	{
		...
		pr_depth--;
		QC_RunError (progfuncs, "PR_ExecuteProgram: locals stack overflow\n");
	}
...
====================
*/
int QC_LeaveFunction (progfuncs_t *progfuncs)
{
	int		i, c;
...
// restore locals from the stack
	...
	if (localstack_used < 0)
		QC_RunError (progfuncs, "PR_ExecuteProgram: locals stack underflow\n");
...
// up stack
	pr_depth--;
	QC_MoveParms(progfuncs, pr_stack[pr_depth].progsnum, pr_typecurrent);
	QC_SwitchProgs(progfuncs, pr_stack[pr_depth].progsnum);
...
ddef32_t *ED_FindLocalOrGlobal(progfuncs_t *progfuncs, char *name, qceval_t **val)
{
	static ddef32_t def;
	ddef32_t *def32;
	ddef16_t *def16;
	...
	case 24:
		...
		{
			def16 = QC_GlobalAtOfs16(progfuncs, pr_xfunction->parm_start+i);
			if (!def16)
				continue;
			if (!strcmp(def16->s_name+progfuncs->stringtable, name))
			{
				*val = (qceval_t *)&pr_progstate[pr_typecurrent].globals[pr_xfunction->parm_start+i];
	...
	//			*val = (eval_t *)&localstack[localstack_used-pr_xfunction->numparms*4];
		...
		def16 = QC_FindGlobal16(progfuncs, name);
		if (!def16)
			return NULL;
		def.ofs = def16->ofs;
		def.type = def16->type;
	...
	case 32:
		...
		for (i = 0; i < pr_xfunction->numparms; i++)
		{
			def32 = QC_GlobalAtOfs32(progfuncs, pr_xfunction->parm_start+i);
			if (!def32)
				continue;
	...
	//			*val = (eval_t *)&localstack[localstack_used-pr_xfunction->numparms*4];
		...
		def32 = QC_FindGlobal32(progfuncs, name);
		if (!def32)
			return NULL;
		break;
	default:
	...
	*val = (qceval_t *)&pr_progstate[pr_typecurrent].globals[def32->ofs];
	return &def;
}
...
# else
//msvc crap
int VARGS qc_linuxlike_snprintf(char *buffer, int size, const char *format, ...)
{
#undef _vsnprintf
	int ret;
	va_list		argptr;
...
#  define snprintf qc_linuxlike_snprintf
# endif
...
char *QC_EvaluateDebugString(progfuncs_t *progfuncs, char *key)
{
	...
	while(c)
	{
		...
		if (c)*c = '\0';
		fdef = QC_FindField(progfuncs, COM_TrimString(c2));
		if (c)*c = '.';
		if (!fdef)
			return "(Bad string)";
		val = (qceval_t *) (((char *)PROG_TO_EDICT(progfuncs, val->_int)->fields) + fdef->ofs*4);		
		type = fdef->type;
	}
	...
	if (assignment)
	{
		...
		{
		case qct_string:
			*(string_t *)val = QC_StringToProgs(progfuncs, QC_NewString (progfuncs, assignment, 0));
			break;
...
*/
		case qct_entity:
			*(int *)val = EDICT_TO_PROG(progfuncs, QC_EdictNum(progfuncs, atoi (assignment)));
			break;

		case qct_field:
			fdef = QC_FindField (progfuncs, assignment);
			if (!fdef)
			{
		...
		case qct_function:
			{
				...
				func = QC_FindFunction (progfuncs, s, &i, progsnum);
				if (!func)
				{
					assignment[-1] = '=';
	...
	strcpy(buf, QC_ValueString(progfuncs, type, val));
...
static void SetExecutionToLine(progfuncs_t *progfuncs, int linenum)
{
...
int QC_ToggleBreakpoint(progfuncs_t *progfuncs, char *filename, int linenum, int flag)	//write alternate route to work by function name.
{
	int ret=0;
	unsigned int fl;
	unsigned int i;
...
====================
*/
void QC_ExecuteCode (progfuncs_t *progfuncs, int s)
{
	qceval_t	*t, *swtch=NULL;
...
#define RUNAWAYCHECK()							\
	...
	{											\
		...
		while(pr_depth > prinst->exitdepth)		\
			QC_LeaveFunction(progfuncs);		\
		pr_spushed = 0;							\
		return;									\
	}

#define OPA ((qceval_t *)&glob[st->a])
#define OPB ((qceval_t *)&glob[st->b])
#define OPC ((qceval_t *)&glob[st->c])
...
void QC_ExecuteProgram (progfuncs_t *progfuncs, func_t fnum)
{
	...
	if (newprogs != initial_progs)
	{
		...
		QC_MoveParms(progfuncs, newprogs, pr_typecurrent);
		QC_SwitchProgs(progfuncs, newprogs);
	}
...
//		PR_MoveParms(0, pr_typecurrent);
		QC_SwitchProgs(progfuncs, initial_progs);
		return;
	}
	...
	{	// negative statements are built in functions
		...
		QC_MoveParms(progfuncs, initial_progs, pr_typecurrent);
		QC_SwitchProgs(progfuncs, initial_progs);
		return;
	}
...
// make a stack frame
	...
	s = QC_EnterFunction (progfuncs, f, initial_progs);

	tempdepth = prinst->numtempstringsstack;
	QC_ExecuteCode(progfuncs, s);


	QC_MoveParms(progfuncs, initial_progs, pr_typecurrent);
	QC_SwitchProgs(progfuncs, initial_progs);

	QC_FreeTemps(progfuncs, tempdepth);
...
void QC_ResumeThread (progfuncs_t *progfuncs, struct qcthread_s *thread)
{
	...
	if (localstack_used + thread->lstackused > LOCALSTACK_SIZE)
		QC_RunError(progfuncs, "Too many locals on resumtion of QC thread\n");

	if (pr_depth + thread->fstackdepth > MAX_STACK_DEPTH)
		QC_RunError(progfuncs, "Too large stack on resumtion of QC thread\n");


	//do progs switching stuff as appropriate. (fteqw only)
	initial_progs = pr_typecurrent;
	QC_MoveParms(progfuncs, prnum, pr_typecurrent);
	QC_SwitchProgs(progfuncs, prnum);
	...
	if (ls != thread->lstackused)
		QC_RunError(progfuncs, "Thread stores incorrect locals count\n");
