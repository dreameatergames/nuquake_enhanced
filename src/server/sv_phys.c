...
// see if any solid entities are inside the final position
	...
	for (e=1 ; e<sv.num_edicts ; e++, check = NEXT_EDICT(check))
	{
		...
		if (SV_TestEntityPosition (check))
			Con_DPrintf ("entity in invalid position\n");
	}
}
...
//
		...
		{	// go along the crease
			if (numplanes != 2)
			{
//				Con_DPrintf ("clip velocity, numplanes == %i\n",numplanes);
				VectorCopy (vec3_origin, ent->v.velocity);
				return 7;
			}
...
//
	...
	default:
		Con_DPrintf ("SV_Physics_client: bad movetype %i", (int)ent->v.movetype);
	}
...
//
	...
	for (i=0 ; i<sv.num_edicts ; i++, ent = NEXT_EDICT(ent))
	{
		...
		|| ent->v.movetype == MOVETYPE_FLYMISSILE)
			...
			Con_DPrintf ("SV_Physics: bad movetype %i", (int)ent->v.movetype);			
	}
