...
void DrawGLWaterPoly (glpoly_t *p)
{
    ...
    for (i = 0; i < p->numverts - 2; i++)
    {
		...
		gVertexFastBuffer[numwaterverts++] = (glvert_fast_t){.flags = VERTEX, .vert = {pnv[0], pnv[1], pnv[2]}, .texture = {v0[3], v0[4]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
		gVertexFastBuffer[numwaterverts++] = (glvert_fast_t){.flags = VERTEX, .vert = {tv0, tv1, v[2]}, .texture = {v[3], v[4]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
		v += VERTEXSIZE;

		tv0 = v[0] + 8*SIN(v[1]*0.05f+realtime)*SIN(v[2]*0.05f+realtime);
		tv1 = v[1] + 8*SIN(v[0]*0.05f+realtime)*SIN(v[2]*0.05f+realtime);
		gVertexFastBuffer[numwaterverts++] = (glvert_fast_t){.flags = VERTEX_EOL, .vert = {tv0, tv1, v[2]}, .texture = {v[3], v[4]}, .color = {255, 255, 255, 255}, .pad0 = {0}};
    }
}
...
void DrawGLWaterPolyLightmap (glpoly_t *p)
{
	GL_DisableMultitexture();
  	...
  	glDrawArrays(GL_TRIANGLE_FAN, 0, MIN(MAX_POLYGON_SIZE, p->numverts) );
}
...
void R_RenderBrushPoly (msurface_t *fa)
{
	...
	#if defined(_arch_dreamcast) && defined(GL_TEXTURE_FILTER_CONTROL_EXT)
