...
void _glClipLineToNearZ(const Vertex* v1, const Vertex* v2, Vertex* vout, float* t) {
    ...
    /* As specified in PVR TA registers, bkg_plane_depth 0.2f*/
    const float NEAR_PLANE = 0.2f;//NEAR_PLANE_DISTANCE;
    /*
    float denom = MATH_Invert(v2->w - v1->w);
    *t = denom*(NEAR_PLANE - v1->w);
    */
    
    //*t = MATH_Slow_Divide((NEAR_PLANE - v1->w) ,(v2->w - v1->w) );
    ...
    float out = MATH_Fast_Divide((NEAR_PLANE - v1->w), MATH_fabs(denom));
