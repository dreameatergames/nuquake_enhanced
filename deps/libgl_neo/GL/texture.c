...
void APIENTRY glTexImage2D(GLenum target, GLint level, GLint internalFormat,
    ...
    static GLuint conversionBytes = 0;
    static GLuint twiddleBytes = 0;
    static GLubyte* conversionBuffer = NULL;
    static GLubyte* twiddleBuffer = NULL;
    ...
    if(!data) {
        ...
        /* Create an in ram buffer for faster conversion */
        if(conversionBytes < bytes && conversionBuffer){
            free(conversionBuffer);
            conversionBuffer = NULL;
        }

        if(!conversionBuffer){
            conversionBuffer = memalign(0x20, bytes);
    ...
    if(needsTwiddling) {
#if 0
        /* Create an in ram buffer for faster twiddle */
        if(twiddleBytes < bytes && twiddleBuffer){
            free(twiddleBuffer);
            twiddleBuffer = NULL;
        }

        if(!twiddleBuffer){
            twiddleBuffer = memalign(0x20, bytes);
            twiddleBytes = bytes;
        }

        void *backupPtr = targetData;
        targetData = twiddleBuffer;
#endif
        if(level){
        ...
        //FASTCPY(backupPtr, targetData, bytes);
