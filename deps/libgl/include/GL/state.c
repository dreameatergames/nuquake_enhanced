...
static GLboolean DIRECT_BUFFER_ENABLED = GL_FALSE;
...
void _glUpdatePVRTextureContext(pvr_poly_cxt_t* context, GLshort textureUnit) {
    ...
    if(!_glIsMipmapComplete(tx1) && enableMipmaps) {
            /* But actually we dont do any of that, we should, but dont*/
            /* Kazade, i'll leave this to you */
            filter = PVR_FILTER_BILINEAR;
            enableMipmaps = GL_FALSE;
    }
    ...
    if(tx1->data) {
        ...
        if(tx1->isPaletted >= 1) {
            if(_glIsSharedTexturePaletteEnabled()) {
                ...
                if(tx1->isPaletted == 2){
                    context->txr.format |= PVR_TXRFMT_4BPP_PAL((tx1->palette) ? tx1->palette->bank : 0);
                } else {
...
GLAPI void APIENTRY glEnable(GLenum cap) {
    switch(cap) {
        ...
        break;
        case GL_DIRECT_BUFFER_KOS:
            DIRECT_BUFFER_ENABLED = GL_TRUE;
        break;
    default:
        break;
    }
...
GLAPI void APIENTRY glDisable(GLenum cap) {
    switch(cap) {
        ...
        break;
        case GL_DIRECT_BUFFER_KOS:
            DIRECT_BUFFER_ENABLED = GL_FALSE;
        break;
    default:
        break;
    }
