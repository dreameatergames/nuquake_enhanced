...
char *QC_UglyValueString (etype_t type, eval_t *val);
...
evaluateimmediate:
//		return QC_UglyValueString(def->type, (eval_t *)&current_progstate->globals[def->ofs]);
		switch(typen)
		{
		case ev_float:
...
//	char *filename;
	...
	if (!QC_ReallyLoadProgs(progfuncs, fname, -1, &progs, false))
	{
		return false;
	}
