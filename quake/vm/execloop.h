...
#define QCPOINTER(p) (qceval_t *)(p->_int+progfuncs->stringtable)
#define QCPOINTERM(p) (qceval_t *)((p)+progfuncs->stringtable)
...
reeval:
	...
	case OP_NOT_V:
		...
		break;
	case OP_NOT_S:
		OPC->_float = (float)(!(OPA->string) || !*QC_StringToNative(progfuncs, OPA->string));
		break;
	case OP_NOT_FNC:
	...
	case OP_EQ_S:
		...
		{
			if (!OPB->string || !*QC_StringToNative(progfuncs, OPB->string))
				OPC->_float = true;
			else
				OPC->_float = false;
		}
		...
		{
			if (!OPA->string || !*QC_StringToNative(progfuncs, OPA->string))
				OPC->_float = true;
			else
				OPC->_float = false;
		}
		else
			OPC->_float = (float)(!strcmp(QC_StringToNative(progfuncs, OPA->string),QC_StringToNative(progfuncs, OPB->string)));
		break;
	case OP_EQ_E:
...
//==================
	...
	case OP_STOREP_IF:
		if ((unsigned int)OPB->_int >= addressableused)
		{
			...
			QC_RunError (progfuncs, "bad pointer write in %s", progfuncs->stringtable + pr_xfunction->s_name);
		}
		ptr = QCPOINTER(OPB);
	...
	case OP_ADDRESS:
		if ((unsigned)OPA->edict >= (unsigned)maxedicts)
		{
			...
			goto cont;
#else
			QC_RunError (progfuncs, "OP_ADDRESS references invalid entity in %s", progfuncs->stringtable + pr_xfunction->s_name);
#endif
		}
		...
		if (!ed || ed->readonly)
		{
			...
			goto cont;
#else
			QC_RunError (progfuncs, "assignment to read-only entity in %s", progfuncs->stringtable + pr_xfunction->s_name);
#endif
		}
...
//		}
	...
	case OP_LOAD_FNC:
		if ((unsigned)OPA->edict >= (unsigned)maxedicts)
			QC_RunError (progfuncs, "OP_LOAD references invalid entity in %s", progfuncs->stringtable + pr_xfunction->s_name);
		ed = PROG_TO_EDICT(progfuncs, OPA->edict);
#ifdef PARANOID
		NUM_FOR_EDICT(ed);		// make sure it's in range
#endif
		ptr = (qceval_t *)(((int *)edvars(ed)) + OPB->_int + progfuncs->fieldadjust);
		OPC->_int = ptr->_int;
		break;

	case OP_LOAD_V:
		if ((unsigned)OPA->edict >= (unsigned)maxedicts)
			QC_RunError (progfuncs, "OP_LOAD_V references invalid entity in %s", progfuncs->stringtable + pr_xfunction->s_name);
		ed = PROG_TO_EDICT(progfuncs, OPA->edict);
#ifdef PARANOID
		NUM_FOR_EDICT(ed);		// make sure it's in range
#endif
...
//about to switch. needs caching.
		...
		QC_MoveParms(progfuncs, (fnum & 0xff000000)>>24, p);
		QC_SwitchProgs(progfuncs, (fnum & 0xff000000)>>24);
...
if (pr_typecurrent != 0)
{
	QC_MoveParms(progfuncs, 0, pr_typecurrent);
	QC_SwitchProgs(progfuncs, 0);
}
...
//		PR_SwitchProgs((OPA->function & 0xff000000)>>24);
		s = QC_EnterFunction (progfuncs, newf, p);
		st = &pr_statements[s];
...
//		break;
	...
	case OP_CP_ITOF:
		ptr = (qceval_t *)(((qbyte *)sv_edicts) + OPA->_int);
	...
	case OP_LOADA_S:
	case OP_LOADA_FNC:
		ptr = (qceval_t *)(&OPA->_int + OPB->_int);
		OPC->_int = ptr->_int;
		break;
	...
	case OP_FETCH_GBL_FNC:
		i = (int)OPB->_float;
		if(i < 0 || i > ((qceval_t *)&glob[st->a-1])->_int)
		{
			QC_RunError(progfuncs, "array index out of bounds: %s[%d]", QC_GlobalStringNoContents(progfuncs, st->a), i);
		}
		t = (qceval_t *)&pr_globals[(uofs)st->a + i];
		OPC->_int = t->_int;
		break;
	case OP_FETCH_GBL_V:
		i = (int)OPB->_float;
		...
		t = (qceval_t *)&pr_globals[(uofs)st->a
			+((int)OPB->_float)*3];
	...
	case OP_CASE:
		...
		case OP_SWITCH_S:
			...
			if ((!swtch->_int && QC_StringToNative(progfuncs, OPA->string)) || (!OPA->_int && QC_StringToNative(progfuncs, swtch->string)))	//one is null (cannot be not both).
				break;
			if (!strcmp(QC_StringToNative(progfuncs, swtch->string), QC_StringToNative(progfuncs, OPA->string)))
			{
				RUNAWAYCHECK();
				st += (sofs)st->b-1; // -1 to offset the s++
			}
		...
		case OP_SWITCH_V:
			...
			break;
		default:
			QC_RunError (progfuncs, "OP_CASE with bad/missing OP_SWITCH %i", swtchtype);
			break;
		}
		break;
	case OP_CASERANGE:
		...
		case OP_SWITCH_F:
			...
			break;
		default:
			QC_RunError (progfuncs, "OP_CASERANGE with bad/missing OP_SWITCH %i", swtchtype);
		}
		break;
	...
	case OP_GLOAD_FNC:
		pr_xstatement = st-pr_statements;
		QC_RunError(progfuncs, "Extra opcode not implemented\n");
		break;
	...
	case OP_BOUNDCHECK:
		...
		{
			pr_xstatement = st-pr_statements;
			QC_RunError(progfuncs, "Progs boundcheck failed. Value is %i.", OPA->_int);
		}
		break;
/*	case OP_PUSH:
...
*/
	default:					
		...
		pr_xstatement = st-pr_statements;
		QC_RunError (progfuncs, "Bad opcode %i", st->op);
	}
}
