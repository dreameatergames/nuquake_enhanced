...
void Host_WriteConfiguration(void) {
  ...
  // config.cfg cvars
  // Pretty sure this is correct. bruce.
...
void Host_WriteConfiguration(void) {
  ...
  if (host_initialized && !isDedicated) {
    ...
    memcpy(ptr, desc, 16);
    ptr += 16; // VM desc
    memcpy(ptr, playername, 32);
    ptr += 32; // DC desc: playername
    ...
    sprintf(buffer, "%c%c%c%c", (char)((filesize & 0xff) >> 0),
    ...
    sprintf(vmu_path, "/vmu/%c%d/%s_CFG", (int)vmu_port.value + 'a',
            (int)vmu_unit.value, VMU_NAME);
...
void Host_ReadConfiguration(void) {
  ...
  // Build the VMU path
  sprintf(path, "/vmu/%c%d/%s_CFG", (int)vmu_port.value + 'a',
          (int)vmu_unit.value, VMU_NAME);
  path[20] = 0;
...
qboolean Host_FilterTime(float time) {
  ...
  if (host_maxfps.value && !cls.timedemo &&
      realtime - oldrealtime < 1.0 / maxfps)
  ...
  // johnfitz
    ...
    host_frametime = CLAMP((1.0 / 120.0), host_frametime,
                           (1.0 / 10.0)); // johnfitz -- use CLAMP
...
void load_defaultConfig(void) {
  ...
  Cbuf_AddText("echo \"Loading default settings...\"\n"
               "m_side \"0.8\"\n"
               "m_forward \"1\"\n"
               "m_yaw \"0.022\"\n"
               "m_pitch \"0.022\"\n"
...
/* speud - VMU end */
  ...
  if (cls.state != ca_dedicated) {
    ...
    Sbar_Init();
    CL_Init();
#if defined(_WIN32) || defined(__linux__) // on non win32, mouse comes before
                                          // video for security reasons
    IN_Init();
#endif
  }
