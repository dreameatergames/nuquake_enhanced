...
/*
    Name: Ian micheal
    Date: 25/11/24 05:54
    Description: Fixed excessive stack usage in sky texture initialization
*/

#define SKY_SIZE 128
#define SKY_TOTAL_PIXELS (SKY_SIZE * SKY_SIZE)

// Move large array to static storage with alignment
static unsigned __attribute__((aligned(32))) sky_trans_buffer[SKY_TOTAL_PIXELS];
...
void R_InitSky(miptex_t *mt)
{
    ...
    for (i = 0; i < SKY_SIZE; i++)
    {
        for (j = 0; j < SKY_SIZE; j++)
        {
            p = src[i*256 + j + 128];
            rgba = &d_8to24table[p];
            sky_trans_buffer[(i*SKY_SIZE) + j] = *rgba;
            r += ((byte *)rgba)[0];
            g += ((byte *)rgba)[1];
            b += ((byte *)rgba)[2];
        }
    ...
    ((byte *)&transpix)[0] = r/SKY_TOTAL_PIXELS;
    ((byte *)&transpix)[1] = g/SKY_TOTAL_PIXELS;
    ((byte *)&transpix)[2] = b/SKY_TOTAL_PIXELS;
    ((byte *)&transpix)[3] = 0;
    ...
    GL_Bind(solidskytexture);
    glTexImage2D(GL_TEXTURE_2D, 0, gl_solid_format, SKY_SIZE, SKY_SIZE, 0, 
                 GL_RGBA, GL_UNSIGNED_BYTE, sky_trans_buffer);
    ...
    for (i = 0; i < SKY_SIZE; i++)
    {
        for (j = 0; j < SKY_SIZE; j++)
        {
            p = src[i*256 + j];
            if (p == 0)
                sky_trans_buffer[(i*SKY_SIZE) + j] = transpix;
            else
                sky_trans_buffer[(i*SKY_SIZE) + j] = d_8to24table[p];
        }
    }
    ...
    GL_Bind(alphaskytexture);
    glTexImage2D(GL_TEXTURE_2D, 0, gl_alpha_format, SKY_SIZE, SKY_SIZE, 0, 
                 GL_RGBA, GL_UNSIGNED_BYTE, sky_trans_buffer);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
}
